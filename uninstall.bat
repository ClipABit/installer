...
REM LOCALAPPDATA fallback (matches Python get_clipabit_directory logic)
...
REM Remove files — shim first (removes Resolve menu entry)
if exist "%SHIM%" (
    del /f "%SHIM%" && (
        echo   Removed: Bootstrap shim
    ) || (
        echo   Failed to remove: Bootstrap shim
        set /a "failures+=1"
    )
)
if exist "%SHIM%.bak" del /f "%SHIM%.bak" >NUL 2>&1
if exist "%PLUGIN_PKG%" (
    rmdir /s /q "%PLUGIN_PKG%" && (
        echo   Removed: Plugin package
    ) || (
        echo   Failed to remove: Plugin package
        set /a "failures+=1"
    )
)
if exist "%PLUGIN_PKG%.bak" rmdir /s /q "%PLUGIN_PKG%.bak" >NUL 2>&1
if exist "%PYTHON_DIR%" (
    rmdir /s /q "%PYTHON_DIR%" && (
        echo   Removed: Python runtime
    ) || (
        echo   Failed to remove: Python runtime
        set /a "failures+=1"
    )
)
if exist "%PYTHON_DIR%.bak" rmdir /s /q "%PYTHON_DIR%.bak" >NUL 2>&1
if exist "%DEPS_DIR%" (
    rmdir /s /q "%DEPS_DIR%" && (
...
if exist "%DEPS_DIR%.bak" rmdir /s /q "%DEPS_DIR%.bak" >NUL 2>&1
...
if exist "%CONFIG_FILE%.bak" del /f "%CONFIG_FILE%.bak" >NUL 2>&1
...
REM Clean up staging leftovers
if exist "%TEMP%\clipabit-staging" rmdir /s /q "%TEMP%\clipabit-staging" >NUL 2>&1
...
if !failures! gtr 0 (
    echo ============================================================
    echo   Uninstall completed with !failures! error(s^).
    echo ============================================================
) else (
    echo ============================================================
